...
export class EntityActions extends MetadataActionSetClass<ClassMetadata, Constructor>() {
    ...
    static actions = [
        ...
        ifMetadata(
            ...
            transformContext(this.toEntityBuilderContext, [
                withPropertiesMetadata([
                    applyToProperties([
                        ...
                        option(this.isEntityField, (meta, ctx) => {
                            const { innerType, isArray } = getTypeInfo<
                                BaseModelConstructor | BaseModelConstructor[]
                            >(meta.type);
                        ...
                        option(always, (meta, ctx) => {
                            console.warn(
                                `Can't make field for ${ctx.target.name} entity: ${String(
                                    ctx.propertyKey
                                )}`
                            );
                        }),
