import { Constraints, Constructor, optional, Property } from "@garrettmk/class-schema";
import { setClassName } from "./util/set-class-name.util";

export type WhereInput<T extends object, F extends Constraints<T> = Constraints<T>> = Constraints<T> & {
    _and?: F[]
    _or?: F[]
    _not?: F[]
}
...
export function WhereInput<T extends object, F extends Constraints<T> = Constraints<T>>(
    ...
    class GeneratedWhereInputClass extends filterType implements WhereInput<T, F> {
